...
#Analise descritiva por fator 1
  ...
  #Analise descritiva por por fator 3
...
#Anova#
m0 <- lm (dados$prod ~ dados$N * dados$P * dados$K  )  #arrrumar os fatores
...
bartlett.test(m0$res ~ interaction (P, N, K), data = dados) #arrrumar os fatores
...
#Transformação de dados#######  Não houve necessidade
  ...
  #valor aproximado de λ
    boxcox((aov(dados$prod+0.000001~dados$P*dados$N))) #arrumar variavel resposta e os fatores
  
  #Descobrindo o valor exato de  
    bc=boxcox((aov(dados$prod+0.000001~dados$P*dados$N))) #arrumar variavel resposta e os fatores
    bc$x[which.max(bc$y)]
  
#____________________________________________________________________
#Dados transformados####### Não houve necessidade    
    #Anova#
    m_trans <- lm (dados$prod ~ dados$P*dados$N)
    
    #Normalidade dos erros
    shapiro.test(m_trans$res)
    
    #Homogeneidade de variâncias
    bartlett.test(m0$res ~ interaction (P, N), data = dados)
...
#Análise de variância    
    ...
    an=with(dados,fat3.dbc (N, P, K, bloco, prod,   #
                      quali = c(TRUE, TRUE, TRUE), 
                      mcomp = "tukey",
                      fac.names = c("Nitrogênio", "Fosforo", "Potássio"),
                      sigT = 0.05, sigF = 0.05))
